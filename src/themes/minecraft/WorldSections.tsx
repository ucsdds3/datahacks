import { ArrowRight, ArrowUpRight, BookOpen, Braces, Compass, Gem, Lightbulb, LockKeyhole, Pickaxe, Trophy, Users } from "lucide-react";
...
export function About() {
  const ingredients = [Lightbulb, null, Braces, null, Users, null, Braces, null, Lightbulb];
  return <>
    <section className="mc-section mc-about" id="about">
      <div className="mc-container mc-about-grid">
        <div><p className="mc-eyebrow">01 / SPAWN POINT</p><h2>A fresh world.<br />A blank canvas.<br /><em>Your next big idea.</em></h2><p className="mc-body">Welcome to DataHacks 2.0. A student-run data science and machine learning hackathon where curiosity is your most valuable resource.</p><p className="mc-body">Team up, try something new, and turn raw data into something worth sharing. You bring the ideas. We’ll bring the workshops, mentors, and midnight snacks.</p><a href="#schedule" className="mc-text-link">See what’s on the horizon <ArrowRight size={18} /></a></div>
        <div className="mc-crafting-wrap"><div className="mc-panel mc-crafting"><div className="mc-panel-title"><span>Crafting</span><span aria-hidden="true">×</span></div><div className="mc-recipe"><div className="mc-crafting-grid" aria-hidden="true">{ingredients.map((Icon, i) => <span className={`mc-slot mc-ingredient-${i}`} key={i}>{Icon && <Icon size={29} strokeWidth={2.5} />}</span>)}</div><ArrowRight className="mc-craft-arrow" size={32} aria-hidden="true" /><div className="mc-slot mc-result" aria-hidden="true"><span>D</span><b>1</b></div></div><p className="mc-recipe-label">Curiosity + code + your people.</p><div className="mc-inventory-caption">Inventory</div><div className="mc-inventory" aria-hidden="true">{Array.from({ length: 9 }, (_, i) => <div className="mc-slot" key={i}>{i === 0 ? <BookOpen size={23} /> : i === 1 ? <Compass size={23} /> : i === 2 ? <Pickaxe size={23} /> : null}</div>)}</div></div><div className="mc-achievement"><span className="mc-achievement-icon"><Trophy size={26} /></span><span><small>Achievement unlocked!</small><strong>Made something that matters.</strong></span></div></div>
      </div>
    </section>
